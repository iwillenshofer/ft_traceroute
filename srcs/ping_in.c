...
/*   Updated: 2023/07/26 17:25:40 by iwillens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_bool	check_icmp_size(t_ping *ft_ping, t_headers *headers)
{
	...
	headers->data_size = htons(headers->ip->tot_len) - (headers->ip->ihl * 4) - sizeof(t_icmpheader);
	return (true);
}
...
static t_bool	prs_icmp(t_ping *ft_ping, t_headers *headers)
{
	if (!check_icmp_size(ft_ping, headers))
		return (false);
	ft_ping->in.recv.type = headers->icmp->type;
	if (ft_ping->in.recv.type == ICMP_TIME_EXCEEDED)
	{
		printf("TTL\n");
	...
	if (ntohs(headers->icmp->un.echo.id) != (unsigned short)(ft_ping->pid))
		return (false);
	if (ft_ping->in.recv.type != ICMP_ECHOREPLY && ft_ping->in.recv.type != ICMP_ADDRESSREPLY && ft_ping->in.recv.type != ICMP_TIMESTAMPREPLY)
		return false;
...
static void	ping_print(t_ping *ft_ping, t_headers *headers)
{
	...
	if (headers->data_size > sizeof(t_time))
...
void	ping_timestamp(t_ping *ft_ping, t_headers *headers)
{
	...
	if (headers->data_size > sizeof(t_time))
	{
		...
		if (ft_ping->in.time.current < ft_ping->in.time.min  || (ft_ping->in.count == 1))
		...
		ft_ping->in.time.avg = ft_ping->in.time.avg
				+ ((ft_ping->in.time.current - ft_ping->in.time.avg) / ft_ping->in.count);
		...
		ft_ping->in.time.variance = ft_ping->in.time.sum_squares / ft_ping->in.count;
	}
}
...
t_bool	ping_in(t_ping *ft_ping)
{
	...
	{
		ft_puthex_bytes(ft_ping->in.recv.msg.msg_iov->iov_base, 4 * 5);
		ft_puthex_bytes(ft_ping->in.recv.msg.msg_iov->iov_base + (4*5), sizeof(t_icmpheader));
		if (prs_icmp(ft_ping, &headers))
		{
			ft_ping->in.count++;
			if ((ft_ping->in.recv.type == ICMP_ECHOREPLY || ft_ping->in.recv.type == ICMP_ADDRESSREPLY || ft_ping->in.recv.type == ICMP_TIMESTAMPREPLY) && !(ft_ping->in.recv.duplicated))
			{
				ft_ping->in.replies++;
